...
1. **Agentic Memory Design:** one CockroachDB transaction links incident state, immutable event, semantic memory, action proposal, provenance, and outbox receipt.
2. **Technological Implementation:** tenant-prefixed distributed vector retrieval, serializable retry handling, idempotency, row-level TTL, and a read-only MCP audit path.
