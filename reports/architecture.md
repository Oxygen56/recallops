...
2. an immutable event with a previous-hash link;
...
- Tenant scope is included in primary keys, unique constraints, and vector-index prefix columns.
- Retrieval excludes revoked or expired memories at query time; TTL later removes eligible rows.
- Action execution uses compare-and-set revision checks and creates a compensating action for undo.
...
- MCP access is scoped to one cluster and starts read-only; write access is not needed for the operator audit.
