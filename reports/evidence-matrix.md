...
| --- | --- | --- | --- | --- |
| CockroachDB is the persistent memory layer | `sql/schema.sql`, `api/src/repository.ts` | 16 API/database tests | v26.2.5 Basic cluster; 16/16 source-bound live tests; raw `api-cloud-tests.json` + aggregate `cloud-cockroachdb.json` | pending Devpost receipt |
...
| Operational memory safety is measured live | `api/src/safetyEval.ts`; `/v1/evaluations/safety`; judge UI scorecard | repeatable 10-check runner; cleanup is a hard gate | 10/10 in 49.668 s on v26.2.5; cleanup zero; source-bound `safety-evaluation.json` | pending video/demo |
