...
| Managed MCP audits the live ledger | `api/src/mcpAudit.ts`; official eight-tool read-only allowlist; five required checks | credential-redaction, argument fail-closed, and required-tool tests | 8/8 advertised reads and 5/5 required checks verified; temporary key deleted | pending video/demo |
| AWS Lambda runs the API | cold-load-verified CommonJS bundle `api/dist/handler.cjs`; `infra/template.yaml` | bundle loads and exports a handler | pending AWS deployment | pending live URL |
